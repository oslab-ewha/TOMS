...
# Uses *task section from rsm_cycle.conf directly (no generated tasks).
#
# Usage:
# ./run_rsm.sh <network_up> <network_down> <seed>
#
# Parameters:
...
function usage() {
    cat <<EOF
Usage: run_rsm.sh <network_up> <network_down> <seed>
EOF
}
...
EOF
    ...
    grep -A 1000 '^\*task' ./rsm_cycle.conf | tail -n +2 >> $config_file
}
