...
{
	int offloading_rand[offloading_max];
...
static void
assign_taskattrs_offloading(taskattrs_t *taskattrs, unsigned max_value, taskattrs_t *offloading_bool)
{
	int i;

	for (i = 0; i < n_tasks; i++)
	{
		if (offloading_bool->attrs[i] == 1)
		{
			taskattrs->attrs[i] = 0;
			continue;
		}
...
BOOL check_utilpower(gene_t *gene)
{
